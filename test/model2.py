...
def preprocess(file_path: str, input_values: list, index: int):
...
def process_files_concurrently():
    ...
    threads = []
    input_values_list = [None] * len(file_paths)
    for i, file_path in enumerate(file_paths):
        thread = threading.Thread(target=preprocess, args=(file_path, input_values_list, i))
